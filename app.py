...
# worker process management + HTTP client ทั้งหมดแยกไปอยู่ worker_client.py แล้ว
# (Architecture report Medium #2 — worker_client ไม่ import streamlit เลย ทดสอบแยกจาก UI ได้)
# ไฟล์นี้เหลือเฉพาะ UI layer ล้วนๆ
from worker_client import (
    _call_worker_chat,
    _call_worker_draft,
    _call_worker_review_finalize,
    _call_worker_review_target,
    _call_worker_topic_step,
...
# ── Worker readiness UI (ส่วน infra จริงอยู่ worker_client.py — ฟังก์ชันนี้เป็น UI ล้วน
# เลยอยู่ต่อที่ app.py: วาด placeholder/st.stop ระหว่างรอ worker โหลดโมเดล) ─────────────
